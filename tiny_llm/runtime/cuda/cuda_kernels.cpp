...
void CausalAttentionKernel::execute(const void *const *inputs,
  ...
  TINY_LLM_CHECK(new_cache_length <= max_len);
  ...
  const auto *q_bias = bias ? static_cast<const float *>(inputs[8]) : nullptr;
  const auto *k_bias = bias ? static_cast<const float *>(inputs[9]) : nullptr;
  const auto *v_bias = bias ? static_cast<const float *>(inputs[10]) : nullptr;
  const auto *o_bias = bias ? static_cast<const float *>(inputs[11]) : nullptr;

  cuda::gemm_row_major_lt(hidden_state, q_weight, q_bias, q_cache, batch,
                          seq_len, hidden_size, q_head, head_dim, 0, seq_len);
  cuda::apply_rope_inplace(cos, sin, pos_ids, q_cache, batch, q_head, seq_len,
                           head_dim, 0, seq_len);
  cuda::gemm_row_major_lt(hidden_state, k_weight, k_bias, k_cache, batch,
                          seq_len, hidden_size, kv_head, head_dim, cache_length,
                          max_len);
  cuda::apply_rope_inplace(cos, sin, pos_ids, k_cache, batch, kv_head, seq_len,
                           head_dim, cache_length, max_len);
