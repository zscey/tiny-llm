...
struct SizeCalculator {
  ...
  explicit SizeCalculator(CudaPlan &extern_plan) : plan(&extern_plan) {
    relations.resize(plan->tensor_descs.size());
    for (uint32_t i = 0, i_end = plan->tensor_descs.size(); i < i_end; ++i) {
      desc_ptr_to_id[&plan->tensor_descs.at(i)] = i;
    }
  }

  void calculate_relations() {
    // tackle inputs
    for (const auto &[_, info] : plan->input_infos) {
      const auto &desc = plan->tensor_descs.at(info.first);
      relations.at(info.first) = Relation{
          .dependence = static_cast<uint32_t>(info.second.size()),
          .v_block = dynamic_gmp.allocate(
    ...
    for (uint32_t i = 0, i_end = plan->tasks.size(); i < i_end; ++i) {
      ...
      for (const auto *desc_ptr : cur_task.output_descs) {
        ...
        relations.at(desc_id).dependence = plan->tensor_dependence.at(desc_id);
      }
      // visitor
      ...
      // of the initializer
      // 4. [option, rope] in pin mode, the outputs are treated as an
      // initializer.
      ...
      for (const auto *desc_ptr : cur_task.input_descs) {
        ...
        if (cur_relation.dependence > 0) {
          ...
          if (cur_relation.dependence == 0 && !cur_relation.exclusive) {
            dynamic_gmp.deallocate(cur_relation.v_block);
          }
        }
      }
    }
  ...
  void set_v_block(const TensorDesc *from_desc, const TensorDesc *to_desc) {
    ...
    if (from_desc != nullptr) {
      ...
      TINY_LLM_CHECK(RuntimeError, !from_relation.exclusive);
      from_relation.dependence = 0;
      to_relation.v_block = from_relation.v_block;
    } else {
      to_relation.v_block = dynamic_gmp.allocate(
          element_num(to_desc->max_shape) * type_size(to_desc->dtype), kAlign);
    }
    to_relation.exclusive = false;
  }

  void assign_initializer(const TensorDesc *desc) {
    auto &cur_relation = relations.at(desc_ptr_to_id.at(desc));
    cur_relation.dependence = 0;
    cur_relation.v_block = static_gmp.allocate(
        element_num(desc->max_shape) * type_size(desc->dtype), kAlign);
    cur_relation.exclusive = true;
  }
  ...
  void bake_ptrs(BufferPtrs ptrs) {
    for (auto &task : plan->tasks) {
      task.inputs.resize(task.input_descs.size());
      for (uint32_t i = 0, i_end = task.inputs.size(); i < i_end; ++i) {
        const auto &cur_relation =
            relations.at(desc_ptr_to_id.at(task.input_descs.at(i)));
        auto *base_ptr =
            cur_relation.exclusive ? ptrs.static_ptr : ptrs.dynamic_ptr;
        task.inputs.at(i) = base_ptr + cur_relation.v_block.offset;
      }

      task.outputs.resize(task.output_descs.size());
      for (uint32_t i = 0, i_end = task.outputs.size(); i < i_end; ++i) {
        const auto &cur_relation =
            relations.at(desc_ptr_to_id.at(task.output_descs.at(i)));
        auto *base_ptr =
            cur_relation.exclusive ? ptrs.static_ptr : ptrs.dynamic_ptr;
        task.outputs.at(i) = base_ptr + cur_relation.v_block.offset;
      }
      ...
      if (std::holds_alternative<CausalAttentionKernel>(task.kernel)) {
        ...
        TINY_LLM_CHECK(RuntimeError, inner_relation.at(1).exclusive);
        cur_kernel.k_cache = reinterpret_cast<float *>(
            ptrs.static_ptr + inner_relation.at(1).v_block.offset);
        TINY_LLM_CHECK(RuntimeError, inner_relation.at(2).exclusive);
...
struct WeightAssigner {
  CudaPlan *plan{};
  const WeightManagerWrapper *wmw{};

  size_t cur_task_id{};

  WeightAssigner(CudaPlan &extern_plan, const WeightManagerWrapper &extern_wmw)
      : plan(&extern_plan), wmw(&extern_wmw) {}
  ...
  void check_and_copy_weight(const TensorDesc *desc, const void *data_ptr,
    ...
    TINY_LLM_CHECK(RuntimeError, desc->cur_shape == target_shape);
    auto dst_tensor = desc_to_tensor(
        {.type = DeviceType::kCuda, .id = ThreadCudaContexts::GetContext().id},
        *desc, data_ptr);
    const auto src_tensor = slice_view_to_tensor(wmw->get_tensor(desc->name));
...
CudaRuntime::CudaRuntime(CudaPlan plan,
  ...
  size_calculator.calculate_relations();
  dynamic_buffer_ =
      CudaAllocator::Allocate(size_calculator.dynamic_gmp.total_size());
  static_buffer_ =
      CudaAllocator::Allocate(size_calculator.static_gmp.total_size());
  size_calculator.bake_ptrs(
      {.dynamic_ptr = static_cast<std::byte *>(dynamic_buffer_.get_ptr()),
       .static_ptr = static_cast<std::byte *>(static_buffer_.get_ptr())});

  // input ptrs
  for (const auto &[name, info] : plan_.input_infos) {
    const auto &task_ios = info.second;
    TINY_LLM_CHECK(RuntimeError, !task_ios.empty());
...
void check_input(const CudaRuntime &cuda_runtime, const std::string &name,
                 const Tensor &tensor) {
  TINY_LLM_CHECK(RuntimeError, cuda_runtime.plan_.input_infos.contains(name));
  TINY_LLM_CHECK(
      RuntimeError,
      cuda_runtime.plan_.plan_config.named_shape_ranges.contains(name));
  TINY_LLM_CHECK(RuntimeError, cuda_runtime.input_ptrs_.contains(name));

  const auto &desc = cuda_runtime.plan_.tensor_descs.at(
      cuda_runtime.plan_.input_infos.at(name).first);
  TINY_LLM_CHECK(RuntimeError, desc.dtype == tensor.dtype());
  TINY_LLM_CHECK(RuntimeError, desc.cur_shape.size() == tensor.shape().size());
  TINY_LLM_CHECK(RuntimeError, tensor.is_continuous());
  ...
  for (size_t i = 0, i_end = desc.cur_shape.size(); i < i_end; ++i) {
    TINY_LLM_CHECK(RuntimeError, static_cast<size_t>(tensor.shape().at(i)) <=
                                     desc.max_shape.at(i));
    TINY_LLM_CHECK(RuntimeError, static_cast<size_t>(tensor.shape().at(i)) >=
                                     min_shape.at(i));
  }
...
void CudaRuntime::bind_input(const std::string &name, const Tensor &tensor) {
  TINY_LLM_CHECK(RuntimeError, tensor.device().type == DeviceType::kCuda);
  TINY_LLM_CHECK(RuntimeError, tensor.device().id == ctx_.id);
...
void CudaRuntime::cpu_tensor_copy_to_input(const std::string &name,
  ...
  TINY_LLM_CHECK(RuntimeError, (tensor.device().type == DeviceType::kCudaHost ||
...
void CudaRuntime::output_copy_to_cpu_tensor(const std::string &name,
  ...
  TINY_LLM_CHECK(RuntimeError, (tensor.device().type == DeviceType::kCudaHost ||
                                tensor.device().type == DeviceType::kCpu));
  TINY_LLM_CHECK(RuntimeError, plan_.output_infos.contains(name));
