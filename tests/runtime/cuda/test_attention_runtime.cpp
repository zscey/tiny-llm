...
TEST(Runtime, AttentionPaged) {
  ...
  auto cuda_plan = cuda::create_cuda_plan(
      graph, {.named_shape_ranges = {
                  {"hidden_state",
                   {.min_shape = {1, 1, 2048}, .max_shape = {1, 10, 2048}}},
                  {"cos", {.min_shape = {5, 32}, .max_shape = {5, 32}}},
                  {"sin", {.min_shape = {5, 32}, .max_shape = {5, 32}}},
                  {"pos_ids", {.min_shape = {1, 1}, .max_shape = {1, 10}}}}});
  { // Check input infos
    EXPECT_EQ(cuda_plan.input_infos.size(), 4);
    const auto &plan_input_0 = cuda_plan.input_infos.at("hidden_state");
    EXPECT_EQ(std::get<0>(plan_input_0), 0);
  ...
  { // Check tasks
    EXPECT_EQ(cuda_plan.tasks.size(), 1);
    const auto &task0 = cuda_plan.tasks[0];
    EXPECT_EQ(task0.name, "paged_attn");
    EXPECT_TRUE(
        std::holds_alternative<cuda::CausalAttentionPagedKernel>(task0.kernel));
    EXPECT_TRUE(task0.predecessors.empty());
    EXPECT_TRUE(task0.successors.empty());
    EXPECT_EQ(
        task0.input_descs,
...

  {
...

    {
      ...
      {
        cuda_runtime.set_prefill(true);
        Tensor hidden_state({.type = DeviceType::kCpu, .id = 0},
                            DataType::kFloat32, {1, 5, 2048}, true);
        {
          auto *hidden_state_ptr = hidden_state.data<float>();
          for (size_t i = 0, i_end = static_cast<size_t>(3) * 2048; i < i_end;
               ++i) {
            *hidden_state_ptr++ = static_cast<float>(i % 100) / 10000.F;
          }
        ...
        EXPECT_TRUE(request_metas[0].seq_len == 3);
        EXPECT_TRUE(request_metas[0].kv_len == 3);
        EXPECT_TRUE(request_metas[0].kv_pages.size() == 1);
        EXPECT_TRUE(request_metas[0].kv_pages.contains("paged_attn"));
        EXPECT_TRUE(request_metas[0].kv_pages.at("paged_attn") ==
                    std::vector<uint32_t>{0});
        EXPECT_TRUE(request_metas[1].seq_len == 2);
        EXPECT_TRUE(request_metas[1].kv_len == 2);
        EXPECT_TRUE(request_metas[1].kv_pages.size() == 1);
        EXPECT_TRUE(request_metas[1].kv_pages.contains("paged_attn"));
        EXPECT_TRUE(request_metas[1].kv_pages.at("paged_attn") ==
        ...
        Tensor out({.type = DeviceType::kCpu}, DataType::kFloat32, {2}, true);
        cuda_runtime.output_copy_to_cpu_tensor("out", out);
        EXPECT_EQ(out.shape(), (std::vector<int64_t>{1, 5, 2048}));
        {
          const auto *out_ptr = out.data<float>();
          const auto *target_ptr =
              reinterpret_cast<const float *>(swm.get_tensor("res").data);
          for (uint32_t j = 0, j_end = 6144; j < j_end; ++j) {
            EXPECT_TRUE(std::abs(*out_ptr++ - target_ptr[j]) < 1e-5F);
          }
...

      {
...

        {
                              ...
                              DataType::kFloat32, {1, 2, 2048}, true);
          {
            ...
            for (size_t i = 0; i < 2048; ++i) {
              *hidden_state_ptr++ =
                  static_cast<float>((i + static_cast<size_t>(3) * 2048) %
                                     100) /
                  10000.F;
            }
            for (size_t i = 0; i < 2048; ++i) {
              *hidden_state_ptr++ =
                  static_cast<float>((i + static_cast<size_t>(2) * 2048) %
                                     100) /
                  10000.F;
            }
          }
          ...
          {
            *pos_ids.data<uint32_t>() = 3;
            pos_ids.data<uint32_t>()[1] = 2;
          }

          request_metas[0].seq_len = 1;
          request_metas[1].seq_len = 1;
          cuda_runtime.bind_request_meta(request_metas);
          EXPECT_TRUE(request_metas[0].seq_len == 1);
          EXPECT_TRUE(request_metas[0].kv_len == 4);
          EXPECT_TRUE(request_metas[0].kv_pages.size() == 1);
          EXPECT_TRUE(request_metas[0].kv_pages.contains("paged_attn"));
          EXPECT_TRUE(request_metas[0].kv_pages.at("paged_attn") ==
                      std::vector<uint32_t>{0});
          EXPECT_TRUE(request_metas[1].seq_len == 1);
...

        {
                              ...
                              DataType::kFloat32, {1, 2, 2048}, true);
          {
            ...
            for (size_t i = 0; i < 2048; ++i) {
              *hidden_state_ptr++ =
                  static_cast<float>((i + static_cast<size_t>(4) * 2048) %
                                     100) /
                  10000.F;
            }
            for (size_t i = 0; i < 2048; ++i) {
          ...
          {
            *pos_ids.data<uint32_t>() = 4;
            pos_ids.data<uint32_t>()[1] = 3;
          }

          cuda_runtime.bind_request_meta(request_metas);
          EXPECT_TRUE(request_metas[0].seq_len == 1);
          EXPECT_TRUE(request_metas[0].kv_len == 5);
          EXPECT_TRUE(request_metas[0].kv_pages.size() == 1);
          EXPECT_TRUE(request_metas[0].kv_pages.contains("paged_attn"));
          EXPECT_TRUE(request_metas[0].kv_pages.at("paged_attn") ==
                      std::vector<uint32_t>{0});
          EXPECT_TRUE(request_metas[1].seq_len == 1);
          EXPECT_TRUE(request_metas[1].kv_len == 4);
          EXPECT_TRUE(request_metas[1].kv_pages.size() == 1);
          EXPECT_TRUE(request_metas[1].kv_pages.contains("paged_attn"));
          EXPECT_TRUE(request_metas[1].kv_pages.at("paged_attn") ==
                      std::vector<uint32_t>{1});
